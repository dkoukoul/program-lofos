...
**Τρέχουσα κατάσταση**: υλοποιημένα — auth (magic-link login, sessions, `config/leaders.json` ως πηγή αλήθειας για πρόσβαση/απενεργοποίηση, βλ. `src/lib/auth.ts`, `src/lib/authorize.ts`, `src/db/syncLeaders.ts`), δημόσιες σελίδες ανά τμήμα (`src/routes/public.tsx`, `src/lib/schedule.ts`, `src/views/public/`), το διαχειριστικό προγράμματος/δράσεων (`src/routes/admin.tsx`, `src/views/admin/`) — δημιουργία/λίστα/δημοσίευση Προγράμματος, πλήρες wizard δημιουργίας/επεξεργασίας Δράσεων με quick-add συντομεύσεις, overlap check, tracking αλλαγών μετά τη δημοσίευση, email ειδοποιήσεις, οδηγός χρήσης διαχειριστικού (`/admin/help`, `src/views/admin/help.tsx`) και info tips (`src/views/admin/info-tip.tsx`) — και iCal feed ανά τμήμα (`src/routes/ical.ts`, `getPublishedSectionActivities` στο `src/lib/schedule.ts`, κουμπί εγγραφής στη δημόσια σελίδα τμήματος). Ό,τι μένει: token rotation από το διαχειριστικό (§9/§10 architecture doc αναφέρουν τη δυνατότητα, όχι υλοποιημένη ακόμα).
...
| `bun run db:sync-leaders` | συγχρονισμός `leaders` table από `config/leaders.json` (πηγή αλήθειας για πρόσβαση· καμία self-registration) |
